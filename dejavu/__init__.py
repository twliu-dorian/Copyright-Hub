...
class Dejavu:
    ...
    def fingerprint_directory(self, path: str, extensions: str, nprocesses: int = None) -> None:
        ...
        for filename, _ in decoder.find_files(path, extensions):
            ...
            if decoder.unique_hash(filename) in self.songhashes_set:
                ...
                self.results += filename +' 已存在於資料庫中，無需存證。'
                continue
        ...
        while True:
            try:
                ...
                self.results +="Failed fingerprinting"
                # Print traceback because we can't reraise it here
                traceback.print_exc(file=sys.stdout)
            else:
                sid = self.db.insert_song(song_name, file_hash, len(hashes))
                self.results +=str(song_name)+" finish fingerprinting, save as file hash: \""+str(file_hash)+"\" with "+str(len(hashes))+" hashes."
    ...
    def fingerprint_file(self, file_path: str, song_name: str = None) -> None:
        ...
        if song_hash in self.songhashes_set:
            ...
            self.results +=song_name +'已存在於資料庫中，無需存證。'
            return self.results
        else:
            ...
            #self.results +=str(song_name)+"is stored as "+str(file_hash)+"with "+str(hashes)+" hashes."
            self.db.insert_hashes(sid, hashes)
            self.db.set_song_fingerprinted(sid)
            self.__load_fingerprinted_audio_hashes()
            #return self.results
    ...
    def align_matches(self, matches: List[Tuple[int, int]], dedup_hashes: Dict[str, int], queried_hashes: int,
        ...
        for song_id, offset, _ in songs_matches[0:topn]:  # consider topn elements in the result
            ...
            song = {
                ...
                FIELD_FILE_SHA1: song.get(FIELD_FILE_SHA1, None).encode('utf-8')
            }
